...
int XrdxAlienFs::Callout(XrdOucErrInfo &error, char** outbyteresult, char *decodedinbytestream, char* uuidstring, int &sid, int &myworker, struct timeval &tv_select, struct timezone &tz)
  ...
  for (int i=0; i<XrdxAlienFS->AlienWorker; i++)
    {
    if (XrdxAlienFS->WorkerAssigned[i]) { inuse++; }
    }
  ...
  if ( (result-(512*1024)) > XrdxAlienFS->WorkerOutputLen[myworker]) {
    ...
    if (result > (1024*1024*1024))
      ...
      if (!XrdxAlienFS->WorkerOutput[myworker]) { fprintf(stderr, "reallocating failed - out of memory ?\n"); exit(-1); }
      }
  }
  ...
  if ( result > 0) {
    ...
    *outbyteresult = XrdxAlienFS->WorkerOutput[myworker];
    ...
    while ( ( (result-offset) && ( ( rresult=::read(XrdxAlienFS->AlienXI[myworker]->SockNum(), *outbyteresult+offset,result-offset)) >0 ) ) || (errno==EAGAIN) || (errno==EINTR))
      { if (rresult>0) {offset+=rresult; } }
...
*/
{
  ...
  if ((scmd = env.Get("xalien.cmd")))
    ...
    if (!strcmp(scmd, "execute"))
      ...
      char *magic_token = (char *) spc_cipherq_b64decrypt(q,(unsigned char*) env.Get("xalien.magic"), &dectokenlen);
      ...
      ZTRACE(fsctl,"decrypted magic token: >" << magic_token << "< length: " << dectokenlen << "  "  << strlen(magic_token));
      ...
      if ( XrdxAlienFS->auth->GetLifetime(sid)<=0) { return XrdxAlienFs::Emsg(epname, error, ETIME, "execute - session is expired",""); }

      // decrypt the command input
      char* inbytestream=0;
      if (!(inbytestream = env.Get("xalien.inputstream"))) { return XrdxAlienFs::Emsg(epname, error, EINVAL, "execute - no input stream exists",""); }
      ...
      XrdxAlienFS->CodecMutex.Lock();
      unsigned char *decodedinbytestream = spc_cipherq_b64decrypt(q,(unsigned char*) inbytestream, &decodedlen);
      XrdxAlienFS->CodecMutex.UnLock();

      if(!decodedinbytestream) { return XrdxAlienFs::Emsg(epname, error, EINVAL, "execute - cannot decrypt your command",""); }

      ZTRACE(open, "Decrypted command:>" << decodedinbytestream );

      gapi_envhash_t *envhash=getEnvHash((char*)decodedinbytestream);
      if(!envhash) { std::cerr << "failed to extract envhash\n"; exit(2); }
      ...
      XrdxAlienFS->CodecMutex.Lock();
      char *magic_crypto = (char*) spc_cipherq_b64encrypt(q,(unsigned char *)magic_token, strlen(magic_token));
      XrdxAlienFS->CodecMutex.UnLock();

      ZTRACE(open,"encrypted magic token is" << magic_crypto);
      if (magic_token) { delete [] magic_token; }
      answer+=magic_crypto;
      ...
      if (envhash->find("encryptReply")!=envhash->end())
        ...
        ZTRACE(open, "ENCRYPTING COMMAND OUTPUT");
      	...
      	XrdxAlienFS->CodecMutex.Lock();
      	char *encoded= (char*)
    	  spc_cipherq_b64encrypt(q,(unsigned char *)outbyteresult, strlen(outbyteresult));
      	XrdxAlienFS->CodecMutex.UnLock();

	      if (encoded) { outbytestream=encoded; free(encoded); }
        }
      else
...
*/
{
   ...
   //   static const char *epname = "close";
   if (executed) { XrdOfsOss->Unlink(fname.c_str()); }
   fname = "";
...
int XrdxAlienFs::FSctl(const int cmd, XrdSfsFSctl &args, XrdOucErrInfo &error, const XrdSecEntity *client)
{
  ...
  if (cmd!=SFS_FSCTL_PLUGIN) { return SFS_ERROR; }
  ...
  if ((scmd = env.Get("xalien.cmd"))) {
    ...
    if (!strcmp(scmd, "getToken")) {
  	...
  	if (!strlen(outbyteresult)) { return XrdxAlienFs::Emsg(epname, error, EPERM, "authenticate - invalid job token", ""); }
  ...
  else
    {
      ...
      {
  	  if (p1 != STR_NPOS) { clientdn.erase(p1); }
    	}
	...
	if (!outbyteresult) { return XrdxAlienFs::Emsg(epname, error, EPERM, "authenticate - cannot verify role", ""); }
	...
	authorizedrole = (char*) alienuser.c_str();
      ...
      answer += "&xalien.session_expiretime=";
      answer += (int)(auth->GetLifetime(mysid) + time(NULL));
      //      auth->Dumpsids();
...
XrdxAlienFsFile::getEnvHash(char* decodedstr) {
  ...
  XrdxAlienFS->CodecMutex.Lock();
  if(!XrdxAlienFS->codec) { XrdxAlienFS->codec = new TBytestreamCODEC(); }


  gapi_envhash_t *envhash = new gapi_envhash_t;
  char *tmp_decode=strdup(decodedstr);
  if(!tmp_decode) { XrdxAlienFS->CodecMutex.UnLock(); return NULL; }

  if ( !XrdxAlienFS->codec->Decode(tmp_decode,strlen(tmp_decode)))
    { XrdxAlienFS->CodecMutex.UnLock(); return NULL; }
  else
    {
    for(int i=0; i<XrdxAlienFS->codec->GetStreamField(XrdxAlienFS->codec->kENVIR , 0, TBytestreamCODEC::kFieldname);i++)
      {
      ...
      envhash->insert(std::make_pair(XrdxAlienFS->codec->GetFieldName(XrdxAlienFS->codec->kENVIR,0,i), XrdxAlienFS->codec->GetFieldValue(XrdxAlienFS->codec->kENVIR,0,i)));
      }
    }
  free(tmp_decode);
  XrdxAlienFS->CodecMutex.UnLock();
