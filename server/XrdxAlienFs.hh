...
public:
  ...
  int  open(const char *dirName, const XrdSecClientName *client = 0, const char *opaque = 0) { return SFS_ERROR;}
...
public:
  ...
  int            getMmap(void **Addr, off_t &Size) { if (Addr) { Addr = 0; } Size = 0; return SFS_OK; }

  int            read(XrdSfsFileOffset   fileOffset, XrdSfsXferSize preread_sz) {return SFS_OK;}

  XrdSfsXferSize read(XrdSfsFileOffset   fileOffset, char *buffer, XrdSfsXferSize buffer_size) { return XrdOfsFile::read(fileOffset,buffer,buffer_size);}

  int            read(XrdSfsAio *aioparm) { return XrdOfsFile::read(aioparm); }

  XrdSfsXferSize write(XrdSfsFileOffset fileOffset, const char *buffer, XrdSfsXferSize buffer_size) { return SFS_ERROR;}
  ...
  XrdxAlienFsFile(char *user=0, int MonID = 0) : XrdOfsFile(user, MonID) { fname = ""; executed=false; }
...
public:
  ...
  //
  XrdSfsDirectory *newDir(char *user=0, int MonID = 0) {return (XrdSfsDirectory *)new XrdxAlienFsDirectory(user, MonID);}

  XrdSfsFile      *newFile(char *user=0, int MonID=0)  {return (XrdSfsFile *)new XrdxAlienFsFile(user, MonID);}
  ...
  int   chmod   ( const char *Name, XrdSfsMode Mode, XrdOucErrInfo &out_error, const XrdSecEntity *client = 0, const char *opaque = 0) { return SFS_ERROR;}

  int   exists  ( const char *fileName, XrdSfsFileExistence &exists_flag, XrdOucErrInfo &out_error, const XrdSecEntity *client = 0, const char *opaque = 0) { return SFS_ERROR;}

  int   fsctl   ( const int cmd, const char *args, XrdOucErrInfo &out_error, const XrdSecEntity *client = 0) { return SFS_ERROR;}

  int            getStats(char *buff, int blen) {return 0;}

  const char  *getVersion();

  int   mkdir   ( const char *dirName, XrdSfsMode Mode, XrdOucErrInfo &out_error, const XrdSecClientName *client = 0, const char *opaque = 0) { return SFS_ERROR;}

  int   prepare ( XrdSfsPrep &pargs, XrdOucErrInfo &out_error, const XrdSecEntity *client = 0) { return SFS_ERROR;}

  int   rem     ( const char *path, XrdOucErrInfo &out_error,	const XrdSecEntity *client = 0, const char *opaque = 0) { return SFS_ERROR;}

  int   remdir  ( const char *dirName, XrdOucErrInfo &out_error, const XrdSecEntity *client = 0, const char *opaque = 0) { return SFS_ERROR;}

  int   rename  ( const char *oldFileName, const char *newFileName, XrdOucErrInfo &out_error, const XrdSecEntity *client = 0, const char *opaqueO = 0, const char *opaqueN = 0) { return SFS_ERROR;}

  int   stat    ( const char *Name, struct stat *buf, XrdOucErrInfo &out_error, const XrdSecEntity *client = 0, const char *opaque = 0)
                { printf("Calling stat\n"); return SFS_ERROR;}

  int   stat    ( const char *Name, mode_t &mode, XrdOucErrInfo &out_error, const XrdSecEntity *client = 0, const char *opaque = 0)
                { printf("Calling stat\n"); return SFS_ERROR;}

  int   lstat   ( const char *Name, struct stat *buf, XrdOucErrInfo &out_error, const XrdSecEntity *client = 0, const char *opaque = 0) { return SFS_ERROR;}
  ...
  // Common functions
  //
  static  int    Mkpath(const char *path, mode_t mode, const char *info=0, XrdSecEntity* client = NULL, XrdOucErrInfo* error = NULL) { return SFS_ERROR;}

  static  int    Emsg(const char *, XrdOucErrInfo&, int, const char *x, const char *y="");

  int            FSctl ( const int cmd, XrdSfsFSctl &args, XrdOucErrInfo &out_error,	const XrdSecEntity *client = 0);
