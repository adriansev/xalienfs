...
void caught_sigpipe(int signal) { std::cout << "DEBUG: " << getpid() << " caught SIGPIPE\n"; }
  ...
  if (!(pid=fork())) {
    ...
    for (int l=0; l < 2000; l++) {
      ...
      if ( (write(outsocket->SockNum(),outbyteresult, outbytelen) ) != outbytelen)
        { fprintf(stderr,"error: cannot write output to outsocket"); exit(-1); }

      fprintf(stdout, "Send completed!\n");

      // done
      if (res) { C2PERLFreeResult(res); }
      continue;
    }
...
{
  if ( argc <4 ) { fprintf(stderr,"error: usage %s <perl-module> <inputpipedir> <nserver>!\n", argv[0]); exit(-1); }

  const char* perlmodule = argv[1];
  const char* inputpipe =  argv[2];
  nserver = atoi(argv[3]);

  if (nserver >100) { nserver =100; }
  if (nserver <0)   { nserver =1; }
  ...
  if(sigemptyset(&sset)<0) { fprintf(stderr,"unable to execute sigemptyset\n"); }

  if ( sigaddset ( &sset, SIGTERM ) < 0 ) { fprintf(stderr,"error adding SIGTERM to sigset\n"); }
  if ( sigaddset ( &sset, SIGINT  ) < 0 ) { fprintf(stderr,"error adding SIGINT to sigset\n"); }
  if ( sigaddset ( &sset, SIGPIPE ) < 0 ) { fprintf(stderr,"error adding SIGPIPE to sigset\n"); }
  if ( sigprocmask ( SIG_UNBLOCK, &sset, &oldset ) < 0 ) { fprintf(stderr,"error setting sigprocmask\n"); }

  act.sa_handler=&settermination;
  act.sa_flags=0;

  if ( sigaction ( SIGTERM, &act, &oldact ) < 0 ) { fprintf(stderr,"error setting signal handler"); }
  if ( sigaction ( SIGINT,  &act, &oldact ) < 0 ) { fprintf(stderr,"error setting signal handler"); }

  // create initial children
  for (int i=0; i<nserver;i++) { serverpids[i] = makeWorker(perlmodule,inputpipe,i); }
  ...
  for (int i=0; i< nserver; i++)
    {
    if ( (serverpids[i]>0) && (!kill(serverpids[i],0)) )
      {
      fflush(stdout);
